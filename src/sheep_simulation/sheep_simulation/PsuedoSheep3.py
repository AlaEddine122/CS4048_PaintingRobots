...
class BoidsSheep():
    ...
    def generate_unique_coord(existing_coord):
        ...
        while True:
            ...
            if not any(differences[:] < BoidsSheep.SEPARATION_DIST):
                candidate[2] = th
                return candidate
    ...
    def calc_cohesion(self):
        ...
        nbs_coh = BoidsSheep.generate_neighbours(self.coord[:2], BoidsSheep.sheep_pos[:,:2], BoidsSheep.COHESSION_DIST)
        ...
        if nbs_coh_len > 0:
            ...
            coh_vector = (center - self.coord[:2]) * BoidsSheep.COHESSION_WEIGHT
    ...
    def calc_seperation(self):
        ##Seperation
        nbs_sep = BoidsSheep.generate_neighbours(self.coord[:2], BoidsSheep.sheep_pos[:, :2], BoidsSheep.SEPARATION_DIST)
    ...
    def calc_alignment(self):
        ...
        ##Alignment
        nbs_ali = BoidsSheep.generate_neighbours(self.coord, BoidsSheep.sheep_pos, BoidsSheep.ALIGNMENT_DIST)
        nbs_ali = nbs_ali[:, 2]
        ...
        if nbs_ali_len > 0:
            ...
            ali_vector = np.array([np.cos(avg_th), np.sin(avg_th)]) * BoidsSheep.ALIGNMENT_WEIGHT
...
def main():
    ...
    BoidsSheep.sheep_pos = np.stack([[40.5, 39.5, 3.9], [40, 40.5, 3.9], [40, 40, 3.9], [40, 41, 3.9]])
    sheep1.coord = np.array(BoidsSheep.sheep_pos[0])
    sheep2.coord = np.array(BoidsSheep.sheep_pos[1])
    sheep3.coord = np.array(BoidsSheep.sheep_pos[2])
    sheep4.coord = np.array(BoidsSheep.sheep_pos[3])
